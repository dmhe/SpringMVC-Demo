...
import com.study.dmhe.usermanage.util.PropertyFilter.MatchType;
...
public class HibernateDao<T, PK extends Serializable> extends SimpleHibernateDao<T, PK> {
	...
	@SuppressWarnings("unchecked")
	public List<T> findByDetachedCriteria(DetachedCriteria criteria, int firstResult, int maxResults) {
	...
	@SuppressWarnings("unchecked")
	public List<T> findByDetachedCriteria(DetachedCriteria criteria, int firstResult, int maxResults,boolean cacheable) {
		Criteria c = criteria.getExecutableCriteria(this.getSession());
